...
def compute_reward(pep_seqs, prot_seq, pep_encoder, prot_encoder, u_score, device, args):
    ...
    affinity_gate = torch.sigmoid(12.0 * (cos_sim - args.bind_floor))
    ...
    total_reward = (1.0 - u_mix) * cos_sim + u_mix * effective_robust - args.bind_penalty_weight * affinity_penalty
    aux_metrics = {
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for step, target_prot in enumerate(pbar):
            ...
            # [B.  (Reward & Filter)]
            outcome_rewards, reward_metrics = compute_reward(gen_peptides, target_prot, pep_encoder, prot_encoder, u_score, device, args)
            with torch.no_grad():
                ...
                sample_weights = torch.softmax(advantages / args.adv_temperature, dim=0).detach()
