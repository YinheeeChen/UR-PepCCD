...
wait_for_file() {
  local path="$1"
  local label="$2"
  echo "[$(date '+%F %T')] waiting for $label -> $path" >> "$LOG_DIR/remaining_file_monitor.log"
  while [ ! -f "$path" ]; do
    sleep 30
  done
  echo "[$(date '+%F %T')] detected $label" >> "$LOG_DIR/remaining_file_monitor.log"
}
...
eval_model() {
  ...
  local head_path="$4"
  local out_dir="$5"
  echo "[$(date '+%F %T')] evaluating $tag on gpu $gpu" >> "$LOG_DIR/remaining_file_monitor.log"
...
wait_for_file "./checkpoints/UR_PepCCD_MoE/rl_elbo_no_implicit_diffusion.pt" "no_implicit model"
wait_for_file "./checkpoints/UR_PepCCD_MoE/rl_elbo_no_implicit_uncertainty_head.pt" "no_implicit head"
wait_for_file "./checkpoints/UR_PepCCD_MoE/rl_elbo_no_routing_diffusion.pt" "no_routing model"
wait_for_file "./checkpoints/UR_PepCCD_MoE/rl_elbo_no_routing_uncertainty_head.pt" "no_routing head"
...
echo "[$(date '+%F %T')] file monitor completed" >> "$LOG_DIR/remaining_file_monitor.log"
